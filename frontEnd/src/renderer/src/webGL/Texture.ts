export class Texture {
    ...
    constructor(gl: WebGL2RenderingContext, image: ImageBitmap | null, width: number | null, height: number | null, wrapMode: number = gl.REPEAT, filterMode: number = gl.LINEAR, internalFormat: number = gl.RGB, format: number = gl.RGB, generateMips: boolean = false, type: number = gl.UNSIGNED_BYTE) {
        ...
        this.gl.texParameteri(this.gl.TEXTURE_2D, this.gl.TEXTURE_MIN_FILTER, generateMips ? this.gl.LINEAR_MIPMAP_LINEAR : filterMode)
        this.gl.texParameteri(this.gl.TEXTURE_2D, this.gl.TEXTURE_MAG_FILTER, filterMode)
        if(image) // if image isn't null, use image as resource
            this.gl.texImage2D(this.gl.TEXTURE_2D, 0, internalFormat, image.width, image.height, 0, format, type, image)
        else // else use null adn input width, heigh
