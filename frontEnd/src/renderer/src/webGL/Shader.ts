...
export class Shader {
    ...
    setUniform(name: string, value: number | [number, number] | [number, number, number] | [number, number, number, number] | Float32Array): void {
        ...
        if (typeof value === 'number') {
            ...
            else if ((value as number[]).length === 9)
                this.gl.uniformMatrix3fv(location, false, value);
            else if ((value as number[]).length === 16)
                this.gl.uniformMatrix4fv(location, false, value);
            else
                console.error(`Unsupported uniform array size: ${(value as number[]).length}`);
    ...
    private createShader(type: GLenum, sourceCode: string): WebGLShader {
        ...
        if (type === this.gl.VERTEX_SHADER) sourceCode = versionDefinition + '#define VERTEX_SHADER\n' + sourceCode;
        else if (type === this.gl.FRAGMENT_SHADER) sourceCode = versionDefinition + '#define FRAGMENT_SHADER\n' + sourceCode;
    ...
    private createProgram(shaderSourceCode: string): WebGLProgram {
        ...
        if (!this.gl.getProgramParameter(shaderProgram, this.gl.LINK_STATUS)) {
            console.error('An error occurred linking shader stages: ' + this.gl.getProgramInfoLog(shaderProgram));
        }
