...
export default class DemLayer {
  ...
  async onAdd(map: mapboxgl.Map, gl: WebGL2RenderingContext) {
    this.map = map
    this.demVAO = new VertexArray(
      gl,
      new Float32Array([
        // position, uv
        this.pointBottomLeft.x,
        this.pointBottomLeft.y,
        0,
        0,
        0,
        this.pointTopRight.x,
        this.pointBottomLeft.y,
        0,
        1,
        0,
        this.pointTopRight.x,
        this.pointTopRight.y,
        0,
        1,
        1,
    ...
    for (let i = 0; i < 60; i += 1) {
      const waterHeightBitmap = await loadImage(
        'textures/YLWaterHeight/TIN_WaterDepth_' + String(i) + '.png'
      )
      this.demTextures.push(
        new Texture(
          gl,
          waterHeightBitmap,
          null,
  ...
  render(gl: WebGL2RenderingContext, matrix: any) {
    ...
    if (this.count < (this.demTextures.length - 1) * this.deltaStep) {
      this.demLayerProgram.setUniformTexture(
        'uWaterHeight0',
        this.demTextures[Math.trunc(this.count / this.deltaStep)],
        0
      )
      this.demLayerProgram.setUniformTexture(
        'uWaterHeight1',
        this.demTextures[Math.trunc(this.count / this.deltaStep) + 1],
        1
      )
      ...
      this.demLayerProgram.setUniformTexture(
        'uWaterHeight0',
        this.demTextures[this.demTextures.length - 1],
        0
      )
      this.demLayerProgram.setUniformTexture(
        'uWaterHeight1',
